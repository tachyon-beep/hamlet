...
def test_grid3d_relative_encoding_dimensions(grid3d_relative):
    """Relative encoding should return [num_agents, 3] normalized positions."""
    ...
    assert encoded.shape == (3, 3), "Should return [num_agents, 3]"
...
def test_grid3d_relative_encoding_values(grid3d_relative):
    ...
    assert torch.allclose(encoded[0], torch.tensor([0.0, 0.0, 0.0]))
    assert torch.allclose(encoded[1], torch.tensor([1.0, 1.0, 1.0]))
    assert torch.allclose(encoded[2], torch.tensor([3 / 7, 4 / 7, 0.5]))


def test_grid3d_scaled_encoding_dimensions(grid3d_scaled):
    """Scaled encoding should return [num_agents, 6] (normalized + ranges)."""
    positions = torch.tensor([[0, 0, 0], [7, 7, 2]], dtype=torch.long)
    affordances = {}

    encoded = grid3d_scaled.encode_observation(positions, affordances)

    assert encoded.shape == (2, 6), "Should return [num_agents, 6] (3 pos + 3 ranges)"
    assert encoded.dtype == torch.float32
...
def test_grid3d_scaled_encoding_values(grid3d_scaled):
    ...
    # First 3 dims: normalized positions
    assert torch.allclose(encoded[0, :3], torch.tensor([3 / 7, 4 / 7, 0.5]))
...
def test_grid3d_absolute_encoding_dimensions(grid3d_absolute):
    """Absolute encoding should return [num_agents, 3] raw coordinates."""
    positions = torch.tensor([[0, 0, 0], [7, 7, 2]], dtype=torch.long)
    affordances = {}

    encoded = grid3d_absolute.encode_observation(positions, affordances)

    assert encoded.shape == (2, 3), "Should return [num_agents, 3]"
    assert encoded.dtype == torch.float32
...
def test_grid3d_absolute_encoding_values(grid3d_absolute):
    ...
    assert torch.allclose(encoded[0], torch.tensor([0.0, 0.0, 0.0]))
    assert torch.allclose(encoded[1], torch.tensor([7.0, 7.0, 2.0]))
    assert torch.allclose(encoded[2], torch.tensor([3.0, 4.0, 1.0]))


def test_grid3d_get_observation_dim_relative(grid3d_relative):
    """get_observation_dim() should return 3 for relative encoding."""
    assert grid3d_relative.get_observation_dim() == 3


def test_grid3d_get_observation_dim_scaled(grid3d_scaled):
    """get_observation_dim() should return 6 for scaled encoding."""
    assert grid3d_scaled.get_observation_dim() == 6


def test_grid3d_get_observation_dim_absolute(grid3d_absolute):
    """get_observation_dim() should return 3 for absolute encoding."""
    assert grid3d_absolute.get_observation_dim() == 3

...
def validate_bars_yaml(config: dict) -> list[str]:
    ...
    for bar in bars:
        ...
        required_bar_keys = ['name', 'index', 'tier', 'range', 'initial', 'base_depletion', 'description']
...
def validate_cascades_yaml(config: dict) -> list[str]:
    ...
    # Check required top-level keys
    required_keys = ['version', 'description', 'math_type', 'modulations', 'cascades', 'execution_order']
    for key in required_keys:
        if key not in config:
            errors.append(f"Missing required key: {key}")

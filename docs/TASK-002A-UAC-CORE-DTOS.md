# TASK-002A: UAC Contracts - Core DTOs

**Status**: Planned
**Priority**: HIGH (foundational)
**Estimated Effort**: 7-12 hours
**Dependencies**: None
**Enables**: TASK-000 (SubstrateConfig), TASK-003 (ActionConfig), TASK-004 (core schemas), TASK-002B (extends AffordanceConfig)
...
5. **SubstrateConfig** - Spatial substrate configuration (TASK-000 integration)
6. **BarConfig** - Meter definitions (energy, health, etc.) - BASIC version
7. **CascadeConfig** - Meter relationships - BASIC version
8. **ActionConfig** - Action space definitions - BASIC version (TASK-003 integration)
...
**Note**: AffordanceConfig is implemented in BASIC form here (core fields only). TASK-002B extends it with capabilities, effect pipelines, and availability masking.
...
## Extension: TASK-000 Integration (SubstrateConfig)

**Context**: TASK-000 (Configurable Spatial Substrates) introduces new spatial configuration that needs DTO validation.
...
**Context**: TASK-000 requires affordances to support flexible position formats (2D list, 3D list, hex dict, graph int).
...
- [ ] **Cross-file validation** (meter references, affordance IDs) deferred to TASK-004
...
- SubstrateConfig (TASK-000): Included in Phase 2
...
**Enables**:
- TASK-000 (Spatial Substrates) - Needs SubstrateConfig
- TASK-003 (Action Space) - Needs ActionConfig
- TASK-004 (Compiler) - Needs core schemas
- TASK-002B (Capabilities) - Extends AffordanceConfig

---

## Recommendation

Implement TASK-002A **before** TASK-002B. Core DTOs are foundational and enable L0-L3 configs. Capability system (TASK-002B) can be deferred until after core UAC is stable.

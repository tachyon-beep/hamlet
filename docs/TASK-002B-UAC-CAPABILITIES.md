# TASK-002B: UAC Contracts - Capability System

**Status**: Planned
**Priority**: MEDIUM (can defer until L4+)
**Estimated Effort**: 8-12 hours
**Dependencies**: TASK-002A (Core DTOs)
...
**Solution**: Extend AffordanceConfig from TASK-002A with a capability composition system that supports rich interaction patterns without requiring code changes.

---

## Guiding Principles (from TASK-002A)

**Universe Compiler**: DTOs enforce schema at load time, catching errors before training starts.

**No-Defaults Principle**: All behavioral parameters must be explicit. See TASK-002A for full details on why this matters.

**Permissive Semantics, Strict Syntax**: Allow semantically valid edge cases (empty effects, duration=1) but reject syntax errors (negative duration, invalid meter references). See TASK-002A for full philosophy.

**Pattern Consistency with Actions**: Affordances follow same cost/effect patterns as Actions (TASK-003). Both use `costs: [{meter, amount}]` and `effects: [{meter, amount}]` lists.
...
**Validation Rules** (implemented in TASK-004 compiler):
...
**Extended Schema (TASK-002B)**:
```python
class AffordanceConfig(BaseModel):
    # Base fields (from TASK-002A)
    id: str
    name: str
...
**Validation Scope Note**: TASK-002B DTOs validate structure (types, ranges). Cross-file validation (meter references, capability conflicts) happens in TASK-004. See "Validation" section below (lines 763-778) for scope boundaries.
...
3. Add validation for meter references (deferred to TASK-004)
...
**Tasks**:
1. Extend AffordanceConfig from TASK-002A with new fields
...
**NOTE**: Structural validation (field types, ranges) happens in DTOs (this task). Cross-file validation (meter references, capability conflicts) happens in TASK-004 (Compiler).

**TASK-002B Validation Scope**:
...
**Depends on**:
- TASK-002A (Core DTOs) - Extends AffordanceConfig from TASK-002A

**Enables**:
- Advanced curriculum levels (L4-L6)
- TASK-004 (Compiler) - Optional capability validation (cross-file checks)
...
Implement TASK-002B **after** TASK-002A and **after** core UAC system is stable (L0-L3 working with basic DTOs). Capability system enables advanced curriculum levels (L4+) but is not required for basic operation.
